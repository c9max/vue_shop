<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 步骤条区域 -->
      <!-- activeIndex - 0 将activeIndex的字符串类型转化为数值类型 -->
      <el-steps
        :space="200"
      ...
      <el-form
        ...
        label-position="top"
      >
        <!-- v-model="activeIndex"为了实现步骤条和tab栏的数据联动效果 -->
        <el-tabs
          :tab-position="'left'"
          v-model="activeIndex"
...
export default {
  ...
  methods: {
    ...
    // tabs页签即将发生变化时，调用该函数，接收到即将离开标签页的值和即将达到标签页的值
    beforeTabLeave(activeName, oldActiveName) {
      // console.log('即将离开的标签页:' + oldActiveName)
      // console.log('即将达到的标签页:' + activeName)
    ...
    add() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) {
          return this.$message.error('请填写完必要的表单项!')
        }
        // 执行添加的业务逻辑
        // this.addForm.goods_cat = this.addForm.goods_cat.join(',')
        // 注意: 这样转化会报错，转成字符串会跟级联选择器中的数组对象冲突
